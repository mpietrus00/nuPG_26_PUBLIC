	...
	mapModulatorTypes { |modulator1, modulator2, modulator3, modulator4|
		instances.collect { |i|
			modulator1.modType[i].action_({ |m|
				synthesis.trainInstances[i].set(\modulator_type_1, m.value)
			});
			modulator2.modType[i].action_({ |m|
				synthesis.trainInstances[i].set(\modulator_type_2, m.value)
			});
			modulator3.modType[i].action_({ |m|
				synthesis.trainInstances[i].set(\modulator_type_3, m.value)
			});
			modulator4.modType[i].action_({ |m|
				synthesis.trainInstances[i].set(\modulator_type_4, m.value)
			});
		};
	}
}

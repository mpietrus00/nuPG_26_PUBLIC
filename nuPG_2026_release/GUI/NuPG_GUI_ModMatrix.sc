NuPG_GUI_ModMatrix {
	...
	draw {|name, dimensions, modulatorsList, n = 1, mode = \classic|
		...
		// oscos mode: all 16 parameters including overlap
		// classic mode: 13 parameters without overlap
		paramNames = if (mode == \oscos) {
			[
				"_fundamental",
				"_formant_01",
				"_formant_02",
		...
		n.collect{|i|
			...
			modNum.collect{|k|
				modEdit[i][k] = guiDefinitions.nuPGButton(
					...
					.action = {|butt|
				// Only toggle modulator visibility if modulatorsList is provided
				if(modulatorsList.notNil and: { k < modulatorsList.size }) {

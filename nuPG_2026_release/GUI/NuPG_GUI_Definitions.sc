NuPG_GUI_Definitions {
	...
	*initClass {
		layoutConfig = (
			...
			columns: (
				...
				colFM: 1530,  // FM tables column (right of group 3)
			...
			rows: (
				...
				row7: 975     // Bottom row (trainControl, groupsControl)
			),
			...
			sizes: (
				...
				modulator: (width: 320, height: 100),
				...
				modMatrix: (width: 190, height: 400),
			...
			windows: (
				...
				formantOne: [\col2, \row6, \tableSmall],    // aligned with fundamental
				overlapOne: [\col2, \row5, \tableSmall],    // aligned with masking
				panOne: [\col2, \row2, \tableSmall],        // aligned with pulsaret
				ampOne: [\col2, \row0, \tableSmall],        // aligned with envelope
				groupsControl: [\col2, \row7, \controlExtraWide],

				// Column 3: Group 2 tables
				formantTwo: [\col3, \row6, \tableSmall],    // aligned with fundamental
				overlapTwo: [\col3, \row5, \tableSmall],    // aligned with masking
				panTwo: [\col3, \row2, \tableSmall],        // aligned with pulsaret
				ampTwo: [\col3, \row0, \tableSmall],        // aligned with envelope

				// Column 4: Group 3 tables
				formantThree: [\col4, \row6, \tableSmall],  // aligned with fundamental
				overlapThree: [\col4, \row5, \tableSmall],  // aligned with masking
				panThree: [\col4, \row2, \tableSmall],      // aligned with pulsaret
				ampThree: [\col4, \row0, \tableSmall],      // aligned with envelope

				// FM column: FM tables (right of group 3)
				fmBuffer: [\colFM, \row6, \tableSmall],     // aligned with formant
				fmAmount: [\colFM, \row5, \tableSmall],     // aligned with overlap
				fmRatio: [\colFM, \row2, \tableSmall],      // aligned with pan
	...
	*formantThreeViewDimensions {
	...
	//overlap one window dimensions (oscos mode)
	*overlapOneViewDimensions {
		^this.getWindowDimensions(\overlapOne);
	}

	//overlap two window dimensions (oscos mode)
	*overlapTwoViewDimensions {
		^this.getWindowDimensions(\overlapTwo);
	}

	//overlap three window dimensions (oscos mode)
	*overlapThreeViewDimensions {
		^this.getWindowDimensions(\overlapThree);
	}
	...
	*panThreeViewDimensions {
	...
	// Classic mode: pan tables move up to overlap row (row5)
	*panOneViewDimensionsClassic {
		^this.getWindowDimensions(\overlapOne);
	}

	*panTwoViewDimensionsClassic {
		^this.getWindowDimensions(\overlapTwo);
	}

	*panThreeViewDimensionsClassic {
		^this.getWindowDimensions(\overlapThree);
	}
	...
	*ampThreeViewDimensions {
	...
	// Classic mode: amp tables move up to pan row (row2)
	*ampOneViewDimensionsClassic {
		^this.getWindowDimensions(\panOne);
	}

	*ampTwoViewDimensionsClassic {
		^this.getWindowDimensions(\panTwo);
	}

	*ampThreeViewDimensionsClassic {
		^this.getWindowDimensions(\panThree);
	}
	...
	*multiParameterModulationViewDimensions {
	...
	//fm buffer window dimensions
	*fmBufferViewDimensions {
		^this.getWindowDimensions(\fmBuffer);
	}

	//fm amount window dimensions
	*fmAmountViewDimensions {
		^this.getWindowDimensions(\fmAmount);
	}

	//fm ratio window dimensions
	*fmRatioViewDimensions {
		^this.getWindowDimensions(\fmRatio);
	}

...
NuPG_OSCInterface {
	...
	*initClass {
	...
	prSetupResponders {
		...
		// param: 0=fundamental, 1-3=formant, 4-6=overlap, 7-9=pan, 10-12=amp
		responders.add(
		...
		responders.add(
			OSCdef(\nuPG_burst, { |msg, time, addr|
				...
				if (data.data_burst_mask[instance].notNil) {
					data.data_burst_mask[instance][0].value = burst;
					data.data_burst_mask[instance][1].value = rest;
				};
			}, baseAddress ++ "/burst")
		);
		...
		responders.add(
			OSCdef(\nuPG_channel, { |msg, time, addr|
				...
				if (data.data_channel_mask[instance].notNil) {
					data.data_channel_mask[instance][0].value = mask;
					data.data_channel_mask[instance][1].value = center;
				};
			}, baseAddress ++ "/channel")
		);
		...
		responders.add(
			OSCdef(\nuPG_sieve, { |msg, time, addr|
				...
				if (data.data_sieve_mask[instance].notNil) {
					data.data_sieve_mask[instance][0].value = size;
				};
			}, baseAddress ++ "/sieve")
		);
		...
		responders.add(
			OSCdef(\nuPG_probability, { |msg, time, addr|
				...
				if (data.data_probability_mask_singular[instance].notNil) {
					data.data_probability_mask_singular[instance].value = value;
				};
		...
		responders.add(
			OSCdef(\nuPG_offset, { |msg, time, addr|
				...
				if (data.data_groups_offset[instance].notNil) {
					3.do { |i|
						if (msg[i + 2].notNil) {
							data.data_groups_offset[instance][i].value = msg[i + 2].asFloat;
						};
					};
				};
			}, baseAddress ++ "/offset")
		...
		responders.add(
			OSCdef(\nuPG_duration, { |msg, time, addr|
				...
				if (data.data_train_duration[instance].notNil) {
					data.data_train_duration[instance].value = duration;
				};
